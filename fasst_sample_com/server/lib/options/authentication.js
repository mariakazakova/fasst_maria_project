...
const authentication = (() => {
  ...
  const init = (app) => {
    ...
    oauth2server.exchange(oauth2orize.exchange.clientCredentials({ userProperty: 'account' }, function (client, scope, done) {
      let token_access = jwt.sign({}, process.env.JWT_SECRET, { expiresIn });
      let token_refresh = jwt.sign({}, process.env.JWT_SECRET);
      ...
      new BasicStrategy(
        ...
        (clientId, secretKey, done) => {
          process.nextTick(async () => {
            const user = await ApiKeys.findOne(clientId, secretKey);
            if (!R.isNil(user)) {
        ...
        (accessToken, done) => {
          process.nextTick(() => {
            jwt.verify(accessToken, process.env.JWT_SECRET, async (err, decoded) => {
              ...
              const user = await ApiKeys.findOne({ clientId: decoded.clientId });
              if (!R.isNil(user)) {
                done(null, user);
              } else {
                done(null, false);
